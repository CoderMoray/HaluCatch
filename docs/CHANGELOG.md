...
- Add complexity & maintainability evaluation (5th dimension)
- Multi-language code risk scanning (Shell/Go/JS/Ruby/Rust/Perl)
- Add per-language code risk breakdown in reports
- Localize report headers (TL;DR → 一句话总结) + code stats first
- Add quantified complexity metrics table to professional report
- Add conditional branching to instruction density
- Refine code risk stats and add weight column
- 护栏新增 3 项检测，覆盖输出不稳定/擅自做主/静默吞错痛点
- 输出确定性新增 Python 渲染函数名兜底检测
- 代码风险标注 skills/ 子目录为 [外部Skill]
- Shell 参数缺失增加函数作用域追踪
- Shell 参数缺失增加 while $# 保护域检测
- Config.yaml skills_is_external 机制

### Changed

- Simplify code risk score to per-file count
- Simplify code stats table header and columns
- Clarify code stats table headers and column names
- Use intuitive emoji summary instead of jargon
- Move summary to top of standard report
- Split heading and ref metrics into sub-indicators
- Remove script_path_refs (redundant with cross_file_deps)
- Split ref metrics into doc/script depth+breadth pairs
- Humorous labels for code/doc ratio, code-scarcity is bad
- 6-tier code/doc ratio with finer granularity
- Fix ruff warnings in web/build.py and build-standalone.py
- Bump ruff target-version to py312, revert chr(10) to \n
- Update web previews to 5-dimension report format
- Update hero subtitle to 5-dimension two-line layout
- Shorten hero subtitle with parallel verbs
- 五维自动审查 → 五维审查
- 1:1 reproduction of real HaluCatch self-review in web previews
- Move demo after report preview, unify scroll animations
- Global staggered data-anim delays, remove IntersectionObserver
- Rebuild docs (html bg fix + code risk stats)
- 输出确定性从文字声明改为模板文件检测
- 超长行从 warn 降为 info 级别
- 代码风险按语言分组展示，info 项从标准版移除
- ℹ️ 代码风格提示从列表改为表格展示
- 代码风格提示→其他提示，代码维度 emoji 🤖→⚙️
- 代码 emoji ⚙️→💻
- 复杂度标题下加 得分越低越好 说明
- SKILL.md 添加报告审查前检查步骤（⚠️ 疑似外部 Skill）
- 复杂度「乘数」→「折扣」，人数看得懂的表述
- 复杂度折扣改为 📋 三行分行展示
- 折扣行加档位说明，覆盖率与折扣关系一目了然
- 脚本覆盖折扣从手拍阶梯改为 1-√覆盖率
- 移除未使用的 mul 变量
- 复杂度汇总改为三行表，去掉指标平列表
- 复杂度汇总表公式改用 KaTeX $...$ 语法
...
- Add complexity dimension rendering in reports
- Improve division-to-zero detection, reduce false positives
- Remove misleading 'AI may fail to reproduce' warning from code risk
- Complexity crash on empty steps, improve code stats table
- Detect inline code as script references in step coverage
- Precise coverage ratio and instruction density metrics
- Adjust complexity weights per review
- Invert code/doc ratio formula, show doc% with clear labels
- Remove stray > tag breaking security audit section HTML
- Wrap scroll-anim .in with requestAnimationFrame
- Remove stray > in security audit section (regression)
- Add html background to prevent dark overscroll in light mode
- Shell 参数缺失误报，default 写法不应触发
- Shell 参数缺失排除 $0 和函数内 $N 误报
- 删除 mktemp 误报规则，|| true 在 set -e 下跳过
- 未捕获 Promise 规则改为两步验证 + 修复 install.js
- Bump-version.sh 漏了 __init__.py，版本号修复为 1.7.10
- 删除 complexity.py _instruction_density() 死代码（L424-428）
- 默认报告输出路径改为 Skill 文件夹内 halucatch/halucatch/reports/
- 移除「读取全部文件」职责，用否定句禁止 AI 自行扫描
- AI 文件阅读移到语义补充阶段，按需而非全量扫描
- 从模糊词列表删除「通常」——中文常用描述词，非业务规则歧义
