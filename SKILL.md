...
**Q：能批量审查多个 Skill 吗？**
A：当前版本不支持。把一个 Skill 目录放一起审查会导致分类混乱。需要逐个审查。

**Q：审查结果能用来做 Skill 认证吗？**
A：HaluCatch 提供评估参考，但不是认证工具。它帮你发现风险，不代替人工判断。

**Q：遇到报错怎么办？**
A：看「异常处理」章节。90% 的报错是路径写错或文件名不规范。如果解决不了，把报错信息贴给 AI。
