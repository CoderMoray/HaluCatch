...
| 「审查 ./data-skill，重点看数据管道」 | 分类为数据驱动型，侧重地基和代码维度 |
...
如果评估过程中有维度未覆盖（如数据驱动型 Skill 但用户未提供 .py 文件），自检等级降为 ⚠️。
...
📥 用户调用 HaluCatch → 解析目标路径
  ...
  └─ ✅ 路径存在
      ...
      └─ 🔍 标准审查
          ...
          │
          └─ [Phase 4 你] 询问是否修复
              ├─ 修 → 按问题类型生成修复方案 → 生成 feedback 模板
              │   ├─ 无地基 → 生成骨架 .py + validate 模式
              │   ├─ 地基不稳 → 参数化修改指南
              │   └─ 缺护栏 → 补充解读规则
              │
              └─ 不修 / 无回应 → 结束
